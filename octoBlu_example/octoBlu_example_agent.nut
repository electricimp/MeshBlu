// #require "Meshblu.class.nut:1.0.0"

// placeholder til library is implemented
class Meshblu {
...
imp.wakeup(5, function() {
    ...
    meshblu.sendMessage("*", "hello world");

    server.log("Stored data: " + http.jsonencode(deviceData));

    meshblu.subscribeWithTypeFilter(deviceData.uuid, ["received", "broadcast"] function(err, resp, data) {
        if(err) {
            server.log(err);
            return;
        }
        ...
        if (typeof data.payload == "string") {
            msg = http.jsondecode(data.payload);
        } else {
            server.log(http.jsonencode(data.payload));
        }

        if("readingInt" in msg) {
            device.send("readingInt", msg.readingInt);
        }
    });
